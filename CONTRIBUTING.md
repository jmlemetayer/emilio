...
**The message is a story, told to whoever reads the diff next.** They read it first, and it should
leave them able to go through the diff without stopping to work out why any of it is there: what the
problem was, what was done about it, and why that way rather than the obvious way.

**Two or three short paragraphs.** That is the size of a substantial change, not a quota to fill — a
trivial patch is a subject line on its own. Past that a message starts explaining the diff, which
the diff does better, and it stops being read at all. Spend the words on what the code cannot say,
and cut whatever the reader would have seen for themselves.
