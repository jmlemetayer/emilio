...
pub fn watch(sender: UnboundedSender<OsEvent>, directory: &Path) -> Result<FileWatcher> {
    let handle = move |result: notify::Result<notify::Event>| match result {
        Ok(event) => {
            // Every notification, before anything is discarded. If the classifier never sees a
            // write, this says whether one was reported and dropped or never arrived at all.
            tracing::trace!(kind = ?event.kind, paths = ?event.paths, "the watch reported");
    ...
    let mut watcher = notify::recommended_watcher(handle)?;

    watcher.watch(directory, RecursiveMode::NonRecursive)?;
    tracing::debug!(directory = %directory.display(), "watching for writes");
